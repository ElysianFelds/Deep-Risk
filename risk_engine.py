...
def evaluate(symbol: str, side: str, entry: float, stop: float) -> dict:
    ...
    # 4. Short-selling eligibility check (must run before sizing)
    open_positions = state.get("open_positions", {})
    held_qty = open_positions.get(symbol, {}).get("qty", 0)
    is_closing_sell = side == "SELL" and held_qty > 0
    is_new_short = side == "SELL" and held_qty <= 0

    if is_new_short and not config.ALLOW_SHORT_POSITIONS:
        result["status"] = "BLOCKED"
        result["reason"] = (f"Short selling is disabled (no open long position in {symbol} to "
                             f"close, and this account doesn't support short selling) — "
                             f"toggle this in menu option 10 if that changes")
        return result

    # 5. Max open positions (only relevant when opening something new)
    if (side == "BUY" or is_new_short) and symbol not in open_positions \
            and len(open_positions) >= config.MAX_OPEN_POSITIONS:
        result["status"] = "BLOCKED"
        result["reason"] = f"Already at max open positions ({config.MAX_OPEN_POSITIONS})"
        return result

    # 6. Sizing: a closing sell just closes what you actually hold — it is
    # NOT re-sized off the risk formula, which is only for NEW entries.
    if is_closing_sell:
        result["suggested_qty"] = held_qty
        result["reason"] = f"Closing existing long position ({held_qty} shares)"
        return result

    # 7. Position sizing for a NEW entry (BUY, or a short if enabled) —
    # risk % of equity / stop distance, then concentration cap
    if stop is None:
        result["status"] = "BLOCKED"
