...
# --- Short selling ---
# Most self-directed Fidelity accounts (and this engine's default) do NOT
# support short selling. When False, the risk engine blocks any SELL signal
# that would open a brand-new short position. A SELL that closes a long
# position you already hold is always allowed regardless of this setting —
# that's a normal exit, not a short. Toggle via menu option 10.
ALLOW_SHORT_POSITIONS = os.getenv("ALLOW_SHORT_POSITIONS", "false").lower() == "true"

# --- Alpaca paper auto-execution ---
# When enabled, every APPROVED new signal is actually submitted as a market
# order to Alpaca's PAPER account (simulated fills, fake money) so you can
# observe what the risk engine's decisions would produce with real execution
# authority — entirely separate from Fidelity, which still requires manual
# execution no matter what this is set to.
#
# HARD SAFETY RULE: this only ever operates when ALPACA_PAPER is true.
# There is no code path in this project that submits a live order.
ALPACA_AUTO_EXECUTE = os.getenv("ALPACA_AUTO_EXECUTE", "false").lower() == "true"
